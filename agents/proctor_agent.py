...
class ProctorAgent:
    ...
    def analyze_frame(self, base64_image):
        ...
        try:
            ...
            if img is None:
                # Could not decode image — don't penalise the student for a bad frame
            ...
            # Detect faces.
            # minNeighbors=6: stricter than default (5) to cut false positives,
            # but not so high (8) that genuine faces in dim lighting are missed.
            # minSize=(40,40): ignores tiny noise but still catches most webcam distances.
            ...
            # Detect phones with a higher confidence threshold
            raw_phone = False
            if self.yolo_loaded:
                ...
                for out in outs:
                    for detection in out:
                        ...
                        # Confidence threshold 0.5: strict enough to avoid everyday
                        # objects being mistaken for phones, but not so high (0.6)
                        # that a partially occluded phone is missed.
                        if confidence > 0.5:
            ...
            # Only report a violation if the same bad condition appears CONFIRM_THRESHOLD
            # frames in a row.  A single frame of looking away is NOT reported.
            if raw_no_face:
